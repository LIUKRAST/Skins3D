...
public final class CustomPlayerModel {
    ...
    private CustomPlayerModel() {}

    public static ModelData getCustomPlayerBasicShape(final Dilation dilation, final float pivotOffsetY) {
        ...
        final ModelPartData c = !Skins3D.configRes4
                ? modelPartData
                : modelPartData.addChild("face_root",
                                ModelPartBuilder.create(),
                                ModelTransform.pivot(0.0f, 0.0f + pivotOffsetY, 0.0f));
        c.addChild(FACE,
                ModelPartBuilder.create()
                        .uv(24, 0)
                        .cuboid(-4.0f, -8.0f, (4.05f * (Skins3D.configRes4 ? 1 : -1)), 8.0f, 8.0f, 0, dilation),
                ModelTransform.pivot(0.0f, 0.0f + pivotOffsetY, 0.0f));
    ...
    public static ModelData getCustomPlayerFinalShape(final Dilation dilation, final boolean slim) {
        final ModelData modelData = getCustomPlayerBasicShape(dilation, 0.0f);
        final ModelPartData modelPartData = modelData.getRoot();
        ...
        final ModelPartBuilder SLIM_LEFT_SLEEVE_3D = layering(1, dilation, 48, 48, -1.0f, -2.0f, -2.0f, 3.0f, 12.0f, 4.0f);
        final ModelPartBuilder SLIM_RIGHT_SLEEVE_3D = layering(1, dilation, 40, 32, -2.0f, -2.0f, -2.0f, 3.0f, 12.0f, 4.0f);

        final ModelPartBuilder LEFT_SLEEVE_3D = layering(1, dilation, 48, 48, -1.0f, -2.0f, -2.0f, 4.0f, 12.0f, 4.0f);
        final ModelPartBuilder RIGHT_SLEEVE_3D = layering(1, dilation, 40, 32, -3.0f, -2.0f, -2.0f, 4.0f, 12.0f, 4.0f);

        final ModelPartBuilder LEFT_PANTS_3D = layering(1, dilation, 0, 48, -2.0f, 0.0f, -2.0f, 4.0f, 12.0f, 4.0f);
        final ModelPartBuilder RIGHT_PANTS_3D = layering(1, dilation, 0, 32, -2.0f, 0.0f, -2.0f, 4.0f, 12.0f, 4.0f);
        final ModelPartBuilder JACKET_3D = layering(1, dilation, 16, 32, -4.0f, 0.0f, -2.0f, 8.0f, 12.0f, 4.0f);
    ...
    public static ModelPartBuilder layering(final int amount,
                                            final Dilation dilation,
                                            final int textureX,
                                            final int textureY,
                                            final float offsetX,
                                            final float offsetY,
                                            ...
                                            final float sizeY,
                                            final float sizeZ) {
        final ModelPartBuilder LAYER = ModelPartBuilder.create();
        for (int i = amount * Skins3D.configRes3; i > 0; i--) {
            final float dil = ((float) i / 100) * (25 / (float) Skins3D.configRes3);
